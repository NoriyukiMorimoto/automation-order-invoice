...
Private Function ResolveRootPath() As String
    ...
    candidate = CombinePath(CombinePath(CombinePath(Environ$("OneDriveCommercial"), SYNC_LIBRARY_FOLDER), MASTER_DATA_FOLDER), ROOT_FOLDER_NAME)
    If IsValidRootPath(candidate) Then
        ResolveRootPath = candidate
        Exit Function
    End If

    candidate = CombinePath(CombinePath(CombinePath(CombinePath(Environ$("USERPROFILE"), SYNC_COMPANY_FOLDER), SYNC_LIBRARY_FOLDER), MASTER_DATA_FOLDER), ROOT_FOLDER_NAME)
    If IsValidRootPath(candidate) Then
        ResolveRootPath = candidate
        Exit Function
    End If

    candidate = CombinePath(CombinePath(CombinePath(Environ$("OneDrive"), SYNC_LIBRARY_FOLDER), MASTER_DATA_FOLDER), ROOT_FOLDER_NAME)
    If IsValidRootPath(candidate) Then
        ResolveRootPath = candidate
        Exit Function
    End If
...
Private Function GetMasterDataRootPath(Optional ByVal unitPriceRootPath As String = "") As String
    ...
    parentPath = Left$(rootPath, InStrRev(rootPath, "\") - 1)
    parentFolderName = Mid$(parentPath, InStrRev(parentPath, "\") + 1)
    If StrComp(parentFolderName, MASTER_DATA_FOLDER, vbTextCompare) = 0 Then
        GetMasterDataRootPath = parentPath
    Else
        GetMasterDataRootPath = CombinePath(parentPath, MASTER_DATA_FOLDER)
    End If
End Function
