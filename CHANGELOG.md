...
## 施工会社ブロック 39/42 行目のフォーム様式変更・40/41 行目の表示調整

### #1 39/42行目を frmSubconSelector 様式へ統一
- 39行目（労災保険 加入負担）・42行目（建設リサイクル法 該当有無）の選択フォームを、`frmNumericKeypad` の排他2択モードから **`frmSubconSelector` の単一選択モード**（`ConfigureSingleSectionMode`、**適用/キャンセル**ボタン付き）へ変更。セクション分けなし・高さは選択項目数に合わせて縮小。
- 併せて `frmNumericKeypad` は排他2択コードを除去し **テンキー専用へ回帰**（`clsCheckboxRelay` は frmSubconSelector 専用に）。
### #2 39行目セルの区切を拡大
- 基本情報 39行目セルの表示を「(ON)甲　　　(OFF)乙」のように、甲と乙の間の全角空白を **3個** へ拡大（42行目も同ロジックで拡大）。
### #3 40/41行目の縮小表示
- 基本情報 40/41行目セルに **ShrinkToFit=True** を設定し、セル幅に収まるよう縮小表示（受注者用 F32/F33 は結合セルのため Excel 仕様上 ShrinkToFit は無効）。
### #4 42行目 表示不具合の対策
- 受注者用の反映先セル（H30/J30・M34/R34）を書き込み前に **結合セルの左上へ解決**するよう修正（M34/R34 が結合セルの場合の書込エラーで表示されない事象への対策）。基本情報セルは受注者用書込より先に更新するため、受注者用側でエラーが出ても基本情報表示は保持。

## 受注者用シートの転記実装（mod_OrderTpl_Header.ApplyContractorHeader / mod_FillManagerName）

### #1 受注者用シートへの転記（注文書テンプレート様式）
- これまでスタブ（コピーのみ）だった **`ApplyContractorHeader`** に、注文書テンプレートの受注者用シート様式へ合わせた転記を実装。フォントはすべて **BIZ UDゴシック**。
- 中央揃え: **S1**←施工会社列27行(注文番号) / **Q2**←C2(作成日, 西暦) / **E9**←16行(業者コード) / **A13**←11行(会社名) / **E20**←C10(工事名) / **E22**←C13(都道府県) / **G24**←C15(工期自, 西暦) / **G26**←C16(工期至, 西暦)。
- 右詰: **Q22**←35行(税込) / **Q23**←33行(税抜) / **Q24**←34行(消費税)。
- 日付は西暦「yyyy年m月d日」で日付値として設定。金額・注文番号等は結合セル左上へ書込。
### #2 発注者ブロック（M10/M11/M12, 出張所長リスト参照）
- 参照ファイルは `マスタデータ\出張所長名\{B4}_線路出張所長リスト.xlsm`（年は基本情報 B4）。基本情報 B6(支店)・C6(出張所) を、リストの B列(支店)・C列(出張所) と照合。
- **M10** ← 照合行の H列(住所)。
- **M11** ← 「大鉄工業株式会社」＋全角空白＋K列(基幹出張所)（例: 大鉄工業株式会社　大阪出張所）。
- **M12** ← 照合行の C列(出張所)＝K列(基幹出張所) の場合は「役職(E)＋全角空白＋氏名(F)」、不一致の場合は「出張所(C)＋全角空白＋役職(E)＋全角空白＋氏名(F)」。
- リスト読込は既存の `mod_FillManagerName` のパス解決を流用し、列を 支店/出張所/役職/氏名/住所/基幹出張所（F2/F3/F5/F6/F8/F11）へ拡張した **`GetOfficeChiefInfo`**（セッションキャッシュ・ADO＋ブックfallback）を追加。
### #3 監視範囲
- ヘッダー転記の監視セルに **C13（都道府県）** を追加し、変更時に受注者用 E22 等が再転記されるように。
- 受注者用転記は既存の生成/一括更新フロー（`ApplyVendorSheetHeaders`）から自動実行される。
- 補足: 西暦日付書式=「yyyy年m月d日」、M11/M12 の区切り=全角スペース、M10-12 の配置はテンプレート既定を踏襲（未指定のため）。

## 内訳明細シートの罫線を入力行数に合わせて再描画（mod_OrderTpl_Detail.ApplySummaryBorders）

- 明細部の罫線が行の挿入/削除（`PositionSubtotalRow` の CopyOrigin 複写）で崩れ、外枠（左端A・右端Q・上端）が閉じない・列区切りや行罫線が欠ける事象を修正。
- 転記完了後に **明細範囲（11行目～小計行の直前 × A:Q）の罫線をいったんリセット**し、**外枠(上下左右)＋各行の横罫線(細)＋列区切りの縦罫線(D/G/J/M/P の右)** を入力行数に合わせて毎回引き直すようにした（挿入/削除の残骸に依存しない決定的な描画）。
- 小計～合計の集計ブロック罫線（小計上の二重線・外枠・下端中線など）は従来どおり。

## 内訳明細の罫線をテンプレート様式どおりに再現（修正・mod_OrderTpl_Detail.ApplySummaryBorders）

- 前版は明細部の縦罫線を全消去して細線5本のみ引き直していたため、テンプレートの列区切り（数量｜単価｜金額 等）や金額グループ境界の中線が消える不具合があった。これを修正。
- 注文書テンプレート「内訳明細」シートの実際の罫線を基準に、明細部（11行目～小計直前×A:Q）を毎回リセット→以下で再描画するよう変更:
  - 横罫線=細線、明細部の上端=二重線。
  - 縦罫線(中線): 外枠左(A)・外枠右(Q)・金額グループ境界(F/I/L/O の左)。
  - 縦罫線(細線): その他の列境界(E/G/H/J/K/M/N/P/Q の左)。
- これにより施行指示書/施行通知書のA列に施工会社名が入った状態で新規シートを作成しても、明細行数に合わせて列区切り・外枠・行罫線が正しく引かれる。集計ブロックの罫線は従来どおり。

## 内訳明細の罫線を全体再描画に統一・A列autofit（mod_OrderTpl_Detail）

- `ApplySummaryBorders` を、明細開始行～合計行(小計+4)・A:Q 全体を対象に一括再描画するよう書き換え。前版は明細部だけを描画し、行挿入/削除時や小計行以降の罫線が反映されない場合があった。
- 横罫線: すべて **xlHairline**。ただし **小計行の上=二重線**、**合計行の下=xlMedium**。
- 縦罫線: 外枠左(A)・外枠右(Q)・金額グループ境界(F/I/L/O の左)=**中線**、その他の列境界(E/G/H/J/K/M/N/P/Q の左)=**細線**。追加(挿入)行にも一律適用。
