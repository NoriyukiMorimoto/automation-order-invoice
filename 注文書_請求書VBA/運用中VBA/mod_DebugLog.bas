...
' Immediateウィンドウへのデバッグ出力(Debug.Print)の有効/無効を切り替えるフラグ。
' 既定値はFalse(VBEを開いていない環境でもDebug.Printのオーバーヘッドを避けるため)。
' このフラグを変更してもログの蓄積(mLogs・AppendLogLineToFile・FlushToSheet)には影響しない。
' デバッグ時のみTrueにすると、Immediateウィンドウにも出力される。
