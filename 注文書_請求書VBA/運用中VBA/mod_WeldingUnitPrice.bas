...
Private Function LoadTemotoRatioMap(ByRef loadErrorText As String) As Object
    ...
    If sourceFilePath = "" Then
        loadErrorText = "ファイルが見つかりません: マスタデータ\" & mod_WeldingTemotoMaster.TemotoMasterFilePatternText()
        Exit Function
    End If
    ...
    If adoSheetName = "" Then
        loadErrorText = "シート「" & mod_WeldingTemotoMaster.TemotoMasterSheetNameText() & "」が見つかりません: " & sourceFilePath
        GoTo Cleanup
    End If
    ...
    If rs.EOF Then
        loadErrorText = "シート「" & mod_WeldingTemotoMaster.TemotoMasterSheetNameText() & "」にデータがありません。"
