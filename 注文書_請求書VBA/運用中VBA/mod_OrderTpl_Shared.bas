...
Private mPlaceholderRepairDone As Boolean ' セッション内で防御的な一括補修を実施済みか(Activate毎の全走査を回避)
...
Private Function UnitListContains(ByVal listText As String, ByVal unitText As String) As Boolean
    ...
    ' 単価表・施工指示書の単位は全角英字(ｍ/Ｍ/ｔ)で入ることがあるため半角へ正規化して照合する
...
' 生成済みテンプレート(支店控/受注者用/注文請書)のプレースホルダー数式を一括補修する。
' 生成時(GenerateVendorOrderSheets)・再転記時・F9レイアウト変更時は各シートが個別に補修済みのため、
' 明示呼び出しは Force:=True で必ず実行し、基本情報シートの Activate からの防御的呼び出し(Force省略)は
' セッション初回のみ実行して、シート切替の都度の全走査(体感フリーズの一因)を回避する。
...
Public Sub OrderTplSanitizePlaceholderFormulas(ByVal ws As Worksheet)
    ...
    For Each cell In formulaRange.Cells
        ...
        ' 代表セル(結合の左上)から数式を1回だけ取得し判定へ渡す(MergeArea往復の重複を回避)
