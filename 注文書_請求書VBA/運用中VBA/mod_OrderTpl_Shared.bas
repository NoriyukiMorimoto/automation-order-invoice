...
Private mVendorInfoCache As Object       ' キャッシュキー=業者名 => Array(業者名, 請求者氏名, 担当工事)
Private mBranchOfficeCodeCache As Object ' キャッシュキー=支店名 => 出張所コード
Private mPlaceholderRepairDone As Boolean ' セッション中の二重実行防止用フラグ(Activate時の全シート一括修復済みか)
...
' 条件書(見積の基準書)。テンプレートは工事区分ごとに「条件書(○○工事)」となる。
...
' 生成済みシートの判定用の基準名一覧(通常5シート + 条件書)。
' 条件書は工事区分ごとにテンプレートが分かれており、実際の名称は「条件書(○○工事)」となるため、
' 照合・比較・転記の際はこの一覧で行う(生成対象のコピー枚数は従来通り5枚のまま)。
...
' 工事区分(基本情報10行目)に対応した条件書テンプレートシートを template から探す。
' 「条件書」で始まり工事区分を含むシート名を返す(表記の揺れに対応)。該当なければ Nothing。
...
' 工事区分「軌道工事」の判定テキスト(別紙IIIの限定条件で使用)
...
' 指定の工事区分が軌道工事かどうかを判定する(正規化した上で完全一致)。別紙IIIはこれに該当する場合のみ生成する。
...
' マスタデータフォルダ内のファイルのパスを解決する(共有フォルダ → ブックのフォルダ → ブック位置)
...
Private Function UnitListContains(ByVal listText As String, ByVal unitText As String) As Boolean
    ...
    ' 単位表記は全角半角(㎡/人/個)で揺れることがあるため、半角へ揃えてから照合する
...
' 生成済みテンプレート(内訳明細/受注者/検収書など)に残るプレースホルダー数式を一括修復する。
' 各テンプレート xlsx への直接参照(例: ='内訳明細(○○)'!E20)が #REF! になっている場合、参照先への
' 不一致がブック再計算時に循環参照ダイアログを引き起こすため、VBA転記前に値へ置き換えて防ぐ。
' 対象は生成済みテンプレート(内訳明細/受注者/検収書など)のプレースホルダー数式全て。
' 生成時(GenerateVendorOrderSheets)・転記時・F9レイアウト変更時は各シートを個別に修復済みのため、
' こちらの呼び出しは Force:=True で必ず実行する。基本情報シートの Activate 時の予防的呼び出し(Forceなし)は
' セッション中1回のみ実行し、シート切替の都度の全件走査(処理負荷の増大)は行わない。
...
Public Sub OrderTplSanitizePlaceholderFormulas(ByVal ws As Worksheet)
    ...
    For Each cell In formulaRange.Cells
        ...
        ' 対象セル(結合の代表セル)から数式を1件だけ取得して判定へ渡す(MergeAreaで重複は判定済み)
