...
Private Const SANPAI_KEYWORD As String = "éYîpèàóù"
...
Cleanup:
    ...
    If errorDescription <> "" Then
        MsgBox "é{çHâÔé–ÇÃÉhÉçÉbÉvÉ_ÉEÉìÇê›íËÇ≈Ç´Ç‹ÇπÇÒÇ≈ÇµÇΩÅB" & vbCrLf & _
...
Public Sub SelectSubcontractorForSelection()
    ...
    If lastRow < DATA_START_ROW Then
        MsgBox "ëŒè€ÉfÅ[É^Ç™Ç†ÇËÇ‹ÇπÇÒÅB", vbExclamation
        Exit Sub
    End If
    ...
    If targetColumn = 0 Then
        If mod_Construction_Order_Import.IsWeldingOutputSheet(ws) Then
            MsgBox "ónê⁄âÔé–(AóÒ)Ç‹ÇΩÇÕãOìπéËå≥âÔé–(BóÒ)ÇëIëÇµÇΩèÛë‘Ç≈é¿çsÇµÇƒÇ≠ÇæÇ≥Ç¢ÅB", vbExclamation
        Else
            MsgBox "é{çHâÔé–Çê›íËÇ∑ÇÈçsÇëIëÇµÇƒÇ≠ÇæÇ≥Ç¢(BóÒÇ…êÆóùî‘çÜÇ™Ç†ÇÈçs)ÅB", vbExclamation
        End If
        Exit Sub
    End If
    ...
    If targetRows.Count = 0 Then
        If mod_Construction_Order_Import.IsWeldingOutputSheet(ws) Then
            MsgBox "âÔé–Çê›íËÇ∑ÇÈçsÇëIëÇµÇƒÇ≠ÇæÇ≥Ç¢(CóÒÇ…êÆóùî‘çÜÇ™Ç†ÇÈçs)ÅB", vbExclamation
        Else
            MsgBox "é{çHâÔé–Çê›íËÇ∑ÇÈçsÇëIëÇµÇƒÇ≠ÇæÇ≥Ç¢(BóÒÇ…êÆóùî‘çÜÇ™Ç†ÇÈçs)ÅB", vbExclamation
        End If
        Exit Sub
    ...
    If Not IsArray(names) Then
        MsgBox "äÓñ{èÓïÒÇÃé{çHâÔé–Ç…ëŒâûÇ∑ÇÈã∆é“É}ÉXÉ^AóÒÇÃåÛï‚Ç™å©Ç¬Ç©ÇËÇ‹ÇπÇÒÅB", vbExclamation
        Exit Sub
    End If
    ...
    MsgBox "ëIëÇ≥ÇÍÇΩ" & targetRows.Count & "çsÇ…Åué{çHâÔé–ñº(" & chosen & ")ÅvÇÃé{çHíPâøÇê›íËÇµÇ‹ÇµÇΩÅB", _
           vbInformation
    Exit Sub
