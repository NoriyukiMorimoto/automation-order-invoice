...
'           WriteValidationLists 内の ScheduleOfficeComboBoxPrompt 呼び出しを撤去し、
'           EnableEvents=True 復帰後の Sheet1.cls 側で呼ぶことで
'           EnableEvents=False 中の OnTime 競合を解消。
...
'    戻り値：True = 呼び出し元で ScheduleOfficeComboBoxPrompt を呼ぶ必要あり
'             False = 不要
'    (#19) keepOffice=False のとき、以前は WriteValidationLists 内で
'    ScheduleOfficeComboBoxPrompt を直接呼んでいたが、Worksheet_Change が
'    EnableEvents=False 中に OnTime 予約 → SilentClearBasicInfo でコンボを
'    非表示にする競合が発生していた。呼び出し元（Sheet1.cls）が
'    EnableEvents=True 復帰後に呼ぶよう責務を移動。
'--------------------------------------------------------------------------
...
Private Function WriteValidationLists(ByVal wsInfo As Worksheet, _
    ...
    ' (#19) keepOffice=False かつ候補あり → 呼び出し元でコンボ表示スケジュールが必要
