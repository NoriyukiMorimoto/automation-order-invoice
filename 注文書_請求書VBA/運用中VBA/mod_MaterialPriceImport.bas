...
'    改修内容（#21）：
'      - 同じ支店・出張所で在来線／新幹線の単価適用保線区が複数ある場合、
'        C20 の線区区分に合うマスタ行を優先して選択するよう修正。
'        福井出張所の新幹線単価適用時に在来線側の「敦賀地域鉄道部」を
'        先に拾ってしまい、新幹線フォルダ配下で見つからない問題を解消。
'    改修内容（#22）：
'      - 工事件名に一致する単価表ブックが複数ある場合、最初の1ブックだけでなく
...
Private Function TryLoadUnitPriceMasterRow(ByRef request As UnitPriceRequest, _
    ...
    If matchedCount > 0 Then
        ...
        MsgBox "単価適用線区シートに支店・出張所は見つかりましたが、線区区分に一致する単価適用保線区を特定できませんでした。" & vbCrLf & _
               ...
               "線区区分：" & request.lineType, vbExclamation
        GoTo Cleanup
    End If
